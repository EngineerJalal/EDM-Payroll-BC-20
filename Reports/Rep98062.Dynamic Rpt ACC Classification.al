report 98062 "Dynamic Rpt-ACC Classification"
{
    // version Temp,EDM.HRPY1

    DefaultLayout = RDLC;
    RDLCLayout = 'Reports Layouts/Dynamic Rpt-ACC Classification.rdlc';
    ...
    dataset
    {
        dataitem("Payroll Ledger Entry";"Payroll Ledger Entry")
        {
            ...
            column(TotalCostUSD;TotalCostUSD)
            {
            }
            column(SumTotalCost;SumTotalCost)
            {
            }
            column(SumTotalCostUSD;SumTotalCostUSD)
            {
            }
            ...
            begin
                ...
                if GroupType = GroupType::"Employee No." then
                  begin
                ...
                //Modified to get value in USD - 04.08.2016 : AIM +
                //NetPayACY:=ROUND(CurrExchRate.ExchangeAmtFCYToFCY("Posting Date",'','USD',NetPay),2);//CurrExchRate.ExchangeAmtLCYToFCY(TODAY,GLSetup."Additional Reporting Currency",NetPay,VarCuFactor);

                 //Modified to calculate Amount in ACY 8.11.2016 :
                ...
                if GroupType = GroupType::"Payroll Date" then
                  begin
                    ...
                    //Modified to calculate Amount in ACY 8.11.2016 : A2+ExchangeTotaltoUSD
                ...
                if GroupType = GroupType::"Payroll Date" then
                  begin
                    ...
                    TDeductionsUSD += TotalDeductionUSD;
                          ...
                          if preEmployee <> "Payroll Ledger Entry"."Employee No." then
                            begin
                              TAdditions := 0;
                              TAdditionsUSD := 0;
                              TDeductions := 0;
                ...
                PayDetailLine.RESET;
                PayElementRecord.RESET;
                CLEAR(PayDetailLine);
                CLEAR(PayElementRecord);
                TotalCost := 0;
                ...
                PayDetailLine.SETRANGE("Employee No.","Employee No.");
                 if PayDetailLine.FINDFIRST then
                  repeat
                    PayElementRecord.SETRANGE(Code,PayDetailLine."Pay Element Code");
                      if PayElementRecord.FINDFIRST then
                        begin
                          if PayElementRecord."Accounting Classification" = PayElementRecord."Accounting Classification"::Cost then
                            begin
                              TotalCost += PayDetailLine.Amount;
                            end;
                        end;
                  until PayDetailLine.NEXT = 0;
                SumTotalCost += TotalCost;
                SumTotalCostUSD := PayrollFunction.ExchangeLCYAmountToACY(SumTotalCost);

                if USD_Currency = true then
                  begin
                   TotalCost := PayrollFunction.ExchangeLCYAmountToACY(TotalCost);
                   //SumTotalCost := PayrollFunction.ExchangeLCYAmountToACY(SumTotalCost);
                  end
            end;
    ...
    var
        ...
        SumTotalCost : Decimal;
        SumTotalCostUSD : Decimal;
