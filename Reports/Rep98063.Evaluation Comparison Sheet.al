report 98063 "Evaluation Comparison Sheet"
{
    // version EDM.HRPY2

    DefaultLayout = RDLC;
    RDLCLayout = 'Reports Layouts/Evaluation Comparison Sheet.rdlc';

    dataset
    {
        dataitem("Evaluation Data Main";"Evaluation Data Main")
        {
            RequestFilterFields = "Code",Date,"Employee No",Status,"Evaluation Interval","Evaluation Period";
            ...
            column(EvaPeriodName;EvaPeriodName)
            {
            }
            ...
            dataitem("Evaluation Data KPI";"Evaluation Data KPI")
            {
                ...
                column(KPICategory_EvaluationDataKPI;"Evaluation Data KPI"."KPI Category")
                {
                }
                column(KPIItem_EvaluationDataKPI;"Evaluation Data KPI"."KPI Item")
                {
                }
                ...
                column(KPICategory;KPICategory)
                {
                }
            }
            ...
            begin
                EvaItems.SETRANGE(Code,"Evaluation Data Main"."Evaluation Period");
                if EvaItems.FINDFIRST then
                  EvaPeriodName := EvaItems.Description;
            end;

            trigger OnPreDataItem();
            begin
                "Evaluation Data Main".SETFILTER("Evaluation Interval",'=%1',EvaluationIntervalVar);
            end;
        }
    }
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                field("Evaluation Interval";EvaluationIntervalVar)
                {
    ...
    trigger OnPreReport();
    begin
        if EvaluationIntervalVar = '' then
          ERROR('Enter Evaluation Interval');
    end;

    var
        KPICategory : Text[50];
        EvaDatatbt : Record "Evaluation Data KPI";
        EvaPeriodName : Text[30];
        EvaItems : Record "Evaluation Items";
        EvaluationIntervalVar : Code[20];
