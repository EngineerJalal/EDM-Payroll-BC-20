report 98017 "Payroll R5"
{
    // version EDM.IT,EDM.HRPY1

    DefaultLayout = RDLC;
    RDLCLayout = 'Reports Layouts/Payroll R5.rdl';
    ...
    dataset
    {
        dataitem("Company Information"; "Company Information")
        {
            ...
            column(VarIncomeTax_Control1000000082; VarIncomeTax)
            {
            }
            ...
            column(CompanyInfo__Arabic_Name_; "Company Information"."Arabic Name")
            {
            }
            ...
            begin
                ...
                EmpTbt.RESET;
                EmpTbt.SETFILTER(Declared, '%1|%2', EmpTbt.Declared::"Non-NSSF", EmpTbt.Declared::Declared);
                    ...
                    repeat
                        ...
                        if PayElement.FINDFIRST then begin
                            ...
                            if ((Emp.FINDFIRST) and ((Emp.Declared = Emp.Declared::Declared) OR (Emp.Declared = Emp.Declared::"Non-NSSF"))) then begin
                                // Modified - 20180411:A2-
                                if STRPOS(PayElement."R5 No.", '100') <> 0 then begin
                                    VarEmployee100 += VarAmount;
                                    VarAdmin100 += VarAdmAmount;
                                end;

                                if STRPOS(PayElement."R5 No.", '110') <> 0 then begin
                                    VarEmployee110 += VarAmount;
                                    VarAdmin110 += VarAdmAmount;
                                end;

                                if STRPOS(PayElement."R5 No.", '130') <> 0 then begin
                                    VarEmployee130 += VarAmount;
                                    VarAdmin130 += VarAdmAmount;
                                end;

                                if STRPOS(PayElement."R5 No.", '140') <> 0 then begin
                                    VarEmployee140 += VarAmount;
                                    VarAdmin140 += VarAdmAmount;
                                end;

                                if STRPOS(PayElement."R5 No.", '150') <> 0 then begin
                ...
                PayLedgerEntry.SETFILTER(Declared, '%1|%2', PayLedgerEntry.Declared::"Non-NSSF", PayLedgerEntry.Declared::Declared);
                ...
                PayLedgerEntry.RESET;
                CLEAR(PayLedgerEntry);
                PayLedgerEntry.SETFILTER("Payroll Date", '%1..%2', DMY2DATE(1, 1, DATE2DMY(VarStDate, 3)), DMY2DATE(31, 3, DATE2DMY(VarStDate, 3)));
                if PayLedgerEntry.FINDFIRST then
                    repeat
                        AmountQ1 := AmountQ1 + PayLedgerEntry."Tax Paid";
                    until PayLedgerEntry.NEXT = 0;

                PayLedgerEntry.RESET;
                CLEAR(PayLedgerEntry);
                PayLedgerEntry.SETFILTER("Payroll Date", '%1..%2', DMY2DATE(1, 4, DATE2DMY(VarStDate, 3)), DMY2DATE(30, 6, DATE2DMY(VarStDate, 3)));
                if PayLedgerEntry.FINDFIRST then
                    repeat
                        AmountQ2 := AmountQ2 + PayLedgerEntry."Tax Paid";
                    until PayLedgerEntry.NEXT = 0;

                PayLedgerEntry.RESET;
                CLEAR(PayLedgerEntry);
                PayLedgerEntry.SETFILTER("Payroll Date", '%1..%2', DMY2DATE(1, 7, DATE2DMY(VarStDate, 3)), DMY2DATE(30, 9, DATE2DMY(VarStDate, 3)));
                if PayLedgerEntry.FINDFIRST then
                    repeat
                        AmountQ3 := AmountQ3 + PayLedgerEntry."Tax Paid";
                    until PayLedgerEntry.NEXT = 0;

                PayLedgerEntry.RESET;
                CLEAR(PayLedgerEntry);
                PayLedgerEntry.SETFILTER("Payroll Date", '%1..%2', DMY2DATE(1, 10, DATE2DMY(VarStDate, 3)), DMY2DATE(31, 12, DATE2DMY(VarStDate, 3)));
                if PayLedgerEntry.FINDFIRST then
                    repeat
                        AmountQ4 := AmountQ4 + PayLedgerEntry."Tax Paid";
                    until PayLedgerEntry.NEXT = 0;
    ...
    var
        Text0001: Label 'ÜÔƒ ƒÒÓ×ÏÌ ÜºÔƒÕ Ü¼Õº á¡ºÏ × ¡Ñí ƒÒÓÌÒ×Óƒó ƒÒóÙ ÙÔß×Ù ÌÒÙÕƒ Õ¿ƒ ƒÒó¡ÙÑ';
        ...
        "ƒÒ__Ù__ƒÒÓ_×___ƒ____ƒ_ÕCaptionLbl": Label 'ƒÒ¡Ùº ƒÒÓó×ñá ƒ½óºƒºÕ';
        V300CaptionLbl: Label '300';
        "ƒÒ__Ù__ƒÒÓ_×____ÎÌÕCaptionLbl": Label 'ƒÒ¡Ùº ƒÒÓó×ñá ºÎÌÕ';
        V290CaptionLbl: Label '290';
        "ƒÒÓ_Ó×Ì_ƒÒÓ_Î×ÌCaptionLbl": Label 'ƒÒÓñÓ×Ì ƒÒÓºÎ×Ì';
        ...
        "ƒÒ__Ù_CaptionLbl": Label 'ƒÒ¡Ùº';
        V262CaptionLbl: Label '262';
        "ƒÒÓ_Ó×Ì_ƒÒÓ_Î×ÌCaption_Control1000000274Lbl": Label 'ƒÒÓñÓ×Ì ƒÒÓºÎ×Ì';
        V261CaptionLbl: Label '261';
        ...
        V400CaptionLbl: Label '400';
        "ƒÒ_ƒÊÌ_ÒÒÊ_Ù__CaptionLbl": Label 'ƒÒªƒÊÌ ÒÒÊÙáí';
        "ƒ_ÓƒÒÙ_ƒÒ_×ƒ___×_ƒÒÜ_×_CaptionLbl": Label 'ƒñÓƒÒÙ ƒÒ×ƒóá × ƒÒÜñ×';
        "ƒÒ_ƒ_Ù_CaptionLbl": Label 'ƒÒóƒÙª';
        "ƒÒÏÙÓ_CaptionLbl": Label 'ƒÒÏÙÓí';
        V251CaptionLbl: Label '251';
